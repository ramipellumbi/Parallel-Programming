...
// A and C are stored row-by-row, while the matrix B is stored column-by-column
  ...
  for (i=0; i<N; i++) {
    ...
    for (j=0; j<N; j++,iC++) { // We're going to compute the dot product of row i of A and column j of B.
      ...
      C[iC] = 0.;
      for (k=0; k<N; k++) C[iC] += A[iA+k] * B[jB+k]; // Each iteration on k adds in the product
