...
double matrix_multiply_blocking(double *A, double *B, double *C, int N, int P, int M)
{
    ...
    size_t padded_N = get_padded_value(N);
    size_t padded_M = get_padded_value(M);
    size_t padded_P = get_padded_value(P);

    double *padded_A = pad_matrix(A, N, P, padded_N, padded_P);
    double *padded_B = pad_matrix(B, P, M, padded_P, padded_M);

#pragma omp parallel default(none) shared(padded_N, padded_P, padded_M, padded_A, padded_B, C, M, P, N) private(iA, jB, iC)
    {
// Compute A1 * B1
...
int main(int argc, char **argv)
{
    ...
    // Print a table row
    printf("\n(%d, %d, %d) %9.4f  %f\n", N, P, M, wctime, error);
    write_data_to_file("out/results-omp.csv", "omp-local-non-divisible", N, P, M, BLOCK_SIZE, num_threads, wctime, wctime_blas, error);
